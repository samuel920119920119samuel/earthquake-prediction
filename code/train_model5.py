...
train_x,train_y=pickle.load(open('raw_data\\train_data_rnn.pickle','rb'))

#train_x=np.array(train_x,dtype='float32')
#train_x=train_x.reshape([-1,1000])
#train_x=np.expand_dims(train_x,axis=2)
#train_y=np.array(train_y,dtype='float32')


model = Sequential()
model.add(CuDNNGRU(50,return_sequences=True,input_shape=[1000,1]))
...
model.summary()
history=model.fit(train_x,train_y,epochs=10,batch_size=100,validation_split=0.1,callbacks=[ModelCheckpoint('md_tmp\\md_tmp_{epoch:03d}.h5',monitor='val_categorical_accuracy', period=1)])
