...
def feature(x):
    return np.concatenate((np.mean(x,axis=-1,keepdims=True),np.max(x,axis=-1,keepdims=True),np.min(x,axis=-1,keepdims=True),np.std(x,axis=-1,keepdims=True),np.mean(np.diff(x,axis=-1),axis=-1,keepdims=True),np.expand_dims(kurtosis(x,axis=-1),axis=-1)),axis=-1)
    #return np.concatenate((x.mean(axis=-1),x.max(axis=-1),x.min(axis=-1),x.std(axis=-1),np.diff(x,axis=-1).mean(axis=-1),kurtosis(x,axis=-1)),axis=-1)
    #return [*x.mean(axis=-1),*x.max(axis=-1),*x.min(axis=-1),*x.std(axis=-1),*np.diff(x,axis=-1).mean(axis=-1),*kurtosis(x,axis=-1)]

def full_feature(x):
    #print(x.shape)
    x1,x2=fftri(x)
    return np.concatenate((feature(x),feature(x1),feature(x2)),axis=-1)
...
for i in range(x_raw.shape[0]):
    ...
    if(i%100==99):
        print(str(i+1)+'/'+str(x_raw.shape[0])+':'+str(time.time()-start_time))
        break
...
with open('v_v.pickle', 'wb') as file_pi:
